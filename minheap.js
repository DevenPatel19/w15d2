...
class MinHeap {
    ...
    extract() {
        ...
        // (hot swap) first node, last node = last node, first node
        [this.heap[1], this.heap[this.size()]] = [
            this.heap[this.size()],
            this.heap[1],
        ];
        // remove the last node
        // catching last node into min variable and popping min, var to compare and return
        let min = this.heap.pop();
        // assign current index to the first node. "1" is the index 1
        // (ln 97) catch current in variable to use as reference in while loop (always set to 1)
        let current = 1;
        // while loop to check children
        while (true) {
            //assign children of current node
            let left = current * 2;
            let right = current * 2 + 1;
            // (ln 106) find the smaller node of the 2 children
            let smaller = Math.min(this.heap[left], this.heap[right]);
            // (ln 108) if smaller is less than current, swap smaller and current
            if (this.heap[smaller] < this.heap[current]) {
                [this.heap[smaller], this.heap[current]] = [
                    this.heap[current],
                    this.heap[smaller],
                ];
                // (ln 111) if you swapped, change current to be smaller
                current = smaller;
            } else {
                // loop breaks
                break;
        ...
        return min;
        // else: return until we get to smallest
    }
...
heap.insert(21);
heap.insert(2);
console.log("======== Initial minHeap ========");
heap.printHorizontalTree();
heap.extract();
console.log("======== minHeap after extraction ========");
heap.printHorizontalTree();
